...
```typescript
import { WEAPONS, WeaponType } from '@marathon-manifest/data';

// Get all assault rifles
const assaultRifles = WEAPONS.filter((w) => w.weaponTypes.includes(WeaponType.AssaultRifle));

// Find a specific weapon
const overrunAR = WEAPONS.find((w) => w.slug === 'overrun-ar');
console.log(overrunAR.stats.damage); // 11.5
```
